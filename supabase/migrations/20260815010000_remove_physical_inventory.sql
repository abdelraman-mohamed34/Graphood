-- Graphood distributes digital products without physical inventory limits.
DROP FUNCTION IF EXISTS public.release_order_stock_atomic(uuid, uuid);
DROP FUNCTION IF EXISTS public.assert_checkout_reservation(uuid, uuid);
...
BEGIN
    ...
    -- Serialize only duplicate submissions from the same buyer for the same
    -- digital system. Purchases by different users remain fully concurrent.
...
ALTER TABLE public.systems DROP COLUMN IF EXISTS stock_quantity;
