...
function buildTemplate<T extends EmailEventType>(event: T, payload: EventPayloadMap[T], locale: "ar" | "en"): Template {
    ...
    switch (event) {
        ...
        case "MEMBER_INVITED": {
            ...
            return { subject: ar ? `دعوة للانضمام إلى ${p.tenantName}` : `You're invited to join ${p.tenantName}`, title: ar ? "لقد تمت دعوتك!" : "You're invited!", body: `<p>${escapeHtml(p.inviterName)} ${ar ? "دعاك للانضمام إلى" : "invited you to join"} <strong>${escapeHtml(p.tenantName)}</strong>.</p>${p.message ? `<blockquote>${escapeHtml(p.message)}</blockquote>` : ""}`, cta: { label: ar ? "قبول الدعوة" : "Accept invitation", href: p.acceptUrl } };
        }
