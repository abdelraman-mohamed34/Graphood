...
export default function AcceptInvitationButton({ token, tenant, locale }: AcceptInvitationButtonProps) {
    ...
    return (
        <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center rounded-sm bg-maroon px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-teal focus:outline-none focus:ring-2 focus:ring-teal focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
        >
            {isLoading ? (
                <>
